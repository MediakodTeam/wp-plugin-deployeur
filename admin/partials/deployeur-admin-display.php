...
<div class="relative mkd-wrapper">
	...
	<?php $title = __('Dashboard', 'deployeur'); ?>
	...
	<div class="px-5 pt-2 pb-2 -ml-5 sm:pb-0 lg:px-12">
		...
		<?php if (!$hasValidWebhook) : ?>
			<p class="mt-2 text-sm text-red-500"><?= __('You need to configure your webhook URL to be able to deploy your website. Please review your hosting settings.', 'deployeur') ?></p>
		<?php endif; ?>
	</div>
	...
	<div class="px-5 pt-2 pb-2 mt-12 -ml-5 sm:pb-0 lg:px-12" id="mkd-table-history">
		<div class="md:flex md:items-center md:justify-between">
			<div class="flex-1">
				<h2 class="!text-lg mt-8 md:!text-xl">
					<?= __('Pending changes', 'deployeur') ?>
				</h2>
				<p class="mt-2 text-sm">
					<?= __('Review the history of your last content editions before deploying. This list will be deleted each time you deploys.', 'deployeur') ?>
				</p>
			</div>
		</div>
		...
		<div class="flow-root">
			...
			<?php else : ?>
				...
				<div class="text-center">
					...
					<h3 class="mt-2 mb-0 text-sm font-medium text-gray-900"><?= __('There is nothing here', 'deployeur') ?></h3>
					<p class="mt-1 text-sm text-gray-500"><?= __("All your recent changes have already been deployed, well done!", "deployeur") ?></p>
				</div>
			<?php endif; ?>
		</div>
	</div>
